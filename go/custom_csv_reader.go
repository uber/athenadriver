// This file is a modified version of upstream csvReader. Original source: https://cs.opensource.google/go/go/+/refs/tags/go1.23.2:src/encoding/csv/reader.go
// I had to add support for distinguishing between ,"", and ,, as the first is empty string but the second should be treated as nil
// I implemented that by adding IsValueMissing method and isFieldEmpty field
...
type Reader struct {
	...
	// records whether value on some index was empty (no quotes between commas like a,,c which Athena uses to denote nil value)
	isFieldEmpty []bool
...
func (r *Reader) IsValueMissing(idx int) bool {
	return r.isFieldEmpty[idx]
}
...
func (r *Reader) readRecord(dst []string) ([]string, error) {
	...
	r.isFieldEmpty = r.isFieldEmpty[:0]
...
parseField:
	for {
		...
		if len(line) == 0 || line[0] != '"' {
			...
			// The length of the field is 0 and there was no quote so it must have been case with ,, which should be interpreted as empty value
			r.isFieldEmpty = append(r.isFieldEmpty, len(field) == 0)
			if i >= 0 {
			...
			for {
				...
				if i >= 0 {
					...
					case rn == r.Comma:
						...
						r.isFieldEmpty = append(r.isFieldEmpty, false)
						continue parseField
