---
title: "athenasql - an opensource AWS Athena database driver for Go"
subtitle: "Developed by Uber ATG"
author:
- Henry Fuheng Wu^[henry.wu@uber.com, Uber ATG, 579 20th St, San Francisco,  CA 94107]
- Mingjie Lai^[mlai@uber.com, Uber ATG, 579 20th St, San Francisco, CA  94107]
- Matt Ranney^[mranney@uber.com, Uber ATG, 3011 Smallman Street, Pittsburgh,  PA 15201]
date: '`r format(Sys.time(), "%B %d, %Y")`'
tags: [Athena, Uber, ATG]
header-includes:
- \usepackage{fancyhdr}
- \usepackage{lastpage}
output:
...
<a href="https://codecov.io/gh/uber/athenasql">
  <img src="https://codecov.io/gh/uber/athenasql/branch/master/graph/badge.svg" />
</a>

<img align="right" height="64" src="resources/logo.png">

`athenasql` is a full-featured AWS Athena database driver for Go developed at Uber ATG.
...
- Support multiple AWS authorization methods (\ref{auth})
- Full support of [Athena Basic Data Types](https://docs.aws.amazon.com/athena/latest/ug/data-types.html)
- Full support of [Athena Advanced Type](https://docs.aws.amazon.com/athena/latest/ug/querying-athena-tables.html) for queries with Geospatial identifiers, ML and UDFs (\ref{dtype})
...
- Athena workgroup and tagging support including remote workgroup creation (\ref{wg})
- Go sql's Prepared statement support (\ref{prepare})
- Go sql's `DB.Exec()` and `db.ExecContext()` support(\ref{db-exec})
- Query cancelling support (\ref{query-cancel})
- Mask columns with specific values (\ref{mask})
- Database missing value handling (\ref{missing-value})
- Read-Only mode (\ref{read-only})

## How to setup/install/test `athenasql`


### Prerequisites - AWS Credentials & S3 Query Result Bucket {#prerequisites}
...
In most cases, you need the following 4 prerequisites \footnote{AWS Security Token Service(STS) does authentication with no need of usual credentials.}:
...
The following is the simplest example for demonstration purpose. The source code is available at [dml_select_simple.go](https://github.com/uber/athenasql/blob/master/examples/query/dml_select_simple.go)\footnote{All the code snippets are tested and stored at github(https://github.com/uber/athenasql/tree/master/examples). Please check the link for latest update.}.
...
### Support Multiple AWS Authentication Methods {#auth}
...
The full code is here at [examples/auth.go](https://github.com/uber/athenasql/tree/master/examples/auth.go).

### Full Support of All Data Types {#dtype}
...
### Query With Workgroup and Tag {#wg}
...
###  Prepared Statement Support for Athena DB {#prepare}
...
###  `DB.Exec()` and `DB.ExecContext()` {#db-exec}
...
\footnote{DB.ExecContext() and DB.QueryContext() are the same way.}
...
which is always *-1* for Athena because auto-increment primary key feature is not supported by Athena.\footnote{Even INDEX is not supported in Athena. Please refer to: https://docs.aws.amazon.com/athena/latest/ug/unsupported-ddl.html}
...
### Mask Columns with Specific Values {#mask}
...
### Query Cancellation {#query-cancel}
...
### Missing Value Handling {#missing-value}
...
the [code of conduct](CODE_OF_CONDUCT.md). The athenasql maintainers keep an eye on
...
The following is `athenasql` Package's UML Class Diagram which may help you to
 understand the code. You can also check the reference section
 (\ref{reference}) for some useful materials. 
